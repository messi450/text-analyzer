...
import { Sparkles, Trash2, BarChart3, Wand2, ChevronDown, Activity, Sliders, LogOut, User, Eraser, Sun, Moon } from 'lucide-react';
...
import ExportPanel, { calculateOverallScore, getScoreColor } from '@/components/text-analyzer/ExportPanel';
...
// Text Analysis Class - Works 100% locally, no API needed
class TextAnalyzer {
    constructor(text) { this.text = text; }
    countTotalChars() { return this.text.length; }
    countCharsNoSpaces() { return this.text.replace(/\s/g, '').length; }
    countWords() { return this.text.replace(/[.,!?;:'"()\-\[\]{}]/g, '').split(/\s+/).filter(w => w.length > 0).length; }
    countSentences() { return this.text.split(/[.!?]+/).filter(s => s.trim().length > 0).length; }
    countParagraphs() { const p = this.text.split(/\n\s*\n/).filter(p => p.trim().length > 0); return Math.max(p.length, this.text.trim().length > 0 ? 1 : 0); }
    getWordFrequency() { const w = this.text.toLowerCase().replace(/[.,!?;:'"()\-\[\]{}]/g, '').split(/\s+/).filter(w => w.length > 0); const f = {}; w.forEach(word => f[word] = (f[word] || 0) + 1); return f; }
    getUniqueWordCount() { return Object.keys(this.getWordFrequency()).length; }
    getAllStats() { return { totalChars: this.countTotalChars(), totalCharsNoSpaces: this.countCharsNoSpaces(), totalWords: this.countWords(), totalSentences: this.countSentences(), totalParagraphs: this.countParagraphs(), uniqueWords: this.getUniqueWordCount() }; }
}

// Synonyms dictionary for suggestions
const SYNONYMS = {
...
// Generate local suggestions with positions for highlighting - ALL with click-to-fix!
function generateLocalSuggestions(text, stats, writingStyle) {
    const suggestions = [];
    if (!text || stats.totalWords === 0) return suggestions;

    // Long sentences check - suggest breaking into two
    const sentenceMatches = [...text.matchAll(/[^.!?]+[.!?]+/g)];
    sentenceMatches.forEach((match, i) => {
        const sentence = match[0];
        const words = sentence.split(/\s+/).filter(w => w.length > 0);
        if (words.length > 25) {
            // Find a good break point (near the middle, after a comma or conjunction)
            const midPoint = Math.floor(words.length / 2);
            const breakWords = ['and', 'but', 'or', 'so', 'because', 'which', 'that', 'while', 'although'];
            let breakIndex = midPoint;
            
            // Look for a good break point
            for (let j = midPoint - 5; j <= midPoint + 5 && j < words.length; j++) {
                if (j > 0 && breakWords.includes(words[j].toLowerCase().replace(/,/g, ''))) {
                    breakIndex = j;
                    break;
                }
            }
            ...
            const suggestedText = firstPart.trim().replace(/,?\s*$/, '.') + ' ' + 
    ...
    // Passive voice check - suggest active alternative
    const passivePatterns = [
        { regex: /\b(was|were)\s+(\w+ed)\b/gi, convert: (m, aux, verb) => verb.replace(/ed$/, '') + 's' },
        { regex: /\b(is|are)\s+being\s+(\w+ed)\b/gi, convert: (m, aux, verb) => verb.replace(/ed$/, 's') },
        { regex: /\b(has|have)\s+been\s+(\w+ed)\b/gi, convert: (m, aux, verb) => 'has ' + verb.replace(/ed$/, '') + 'ed' },
    ];
    ...
    // Filler words - click to remove or replace
    ...
    fillers.forEach(filler => {
        ...
        while ((m = regex.exec(text)) !== null) {
            ...
            const suggested = synonyms[0] || ''; // First synonym or empty (remove)
    ...
    // Weak words - suggest stronger alternatives
    ...
    let wordMatch;
    while ((wordMatch = wordRegex.exec(text.toLowerCase())) !== null) {
        const word = wordMatch[0];
        if (!wordPositions[word]) wordPositions[word] = [];
        wordPositions[word].push({ index: wordMatch.index, length: wordMatch[0].length });
    }

    Object.entries(wordPositions).forEach(([word, positions]) => {
        if (positions.length >= 3 && positions.length <= 6) {
            // Find occurrences that are close together (within 200 chars)
            for (let i = 1; i < positions.length; i++) {
                if (positions[i].index - positions[i-1].index < 200) {
                    ...
                    break; // Only flag once per repeated word
                }
            }
        }
    });
    ...
    while ((dsMatch = doubleSpaceRegex.exec(text)) !== null) {
        suggestions.push({
            ...
            suggested: ' ',
            start: dsMatch.index,
            end: dsMatch.index + dsMatch[0].length
        });
    }
    ...
    while ((msMatch = missingSpaceRegex.exec(text)) !== null) {
        suggestions.push({
            id: `mspace-${msMatch.index}`,
            type: 'grammar',
            severity: 'medium',
            title: 'Missing space after punctuation',
            description: 'Add a space after punctuation marks.',
            original: msMatch[0],
            suggested: msMatch[0][0] + ' ' + msMatch[0][1],
            start: msMatch.index,
            end: msMatch.index + msMatch[0].length
        });
    }
...
const DEFAULT_PREFERENCES = { theme: 'light', default_writing_style: 'casual', default_language: 'en', show_suggestions: true, auto_analyze: true, font_size: 'medium' };
...
export default function TextAnalyzerPage() {
    ...
    // Core text analysis - always works locally, no API needed
    const { stats, wordFrequency, readability, sentiment, keywords, localSuggestions, overallScore } = useMemo(() => {
        if (!text.trim()) return { stats: { totalChars: 0, totalCharsNoSpaces: 0, totalWords: 0, totalSentences: 0, totalParagraphs: 0, uniqueWords: 0 }, wordFrequency: {}, readability: null, sentiment: null, keywords: [], localSuggestions: [], overallScore: 0 };
        ...
        const suggestions = generateLocalSuggestions(text, stats, writingStyle);
        ...
        return { stats, wordFrequency, readability: readabilityData, sentiment: sentimentData, keywords: extractKeywords(text, wordFrequency), localSuggestions: suggestions, overallScore: score };
    }, [text, writingStyle]);
    ...
    const handleLoadAnalysis = (a) => { 
        setText(a.original_text || ''); 
        setWritingStyle(a.writing_style || 'casual'); 
        setLanguage(a.language || 'en'); 
    };
    
    const handleUpdatePreferences = (p) => { 
        setPreferences(p); 
        try {
            localStorage.setItem('textalyzer_preferences', JSON.stringify(p));
        } catch {}
    };
    ...
    // Handle applying all fixes at once - fixes are pre-sorted from end to start
    const handleApplyAllFixes = (sortedFixes) => {
        let newText = text;
                        ...
                        <div className="flex items-center gap-0.5 sm:gap-1">
                            ...
                            {text && (
                                <Button 
                                    ...
                                    size="sm" 
                                    onClick={handleClear} 
                                    className="h-8 px-3 border-red-200 bg-red-50 text-red-600 hover:bg-red-100 hover:text-red-700 hover:border-red-300 font-medium gap-1.5 transition-all"
                                >
                ...
                <div className="grid lg:grid-cols-2 gap-5">
                    ...
                    >
                        <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-2xl border border-slate-200/60 dark:border-slate-700/60 p-4 shadow-sm hover:shadow-md transition-shadow">
                            <TextInputWithHighlights
                                onTextChange={setText}
                                text={text}
                                issues={localSuggestions}
                            />
                        </div>
                    ...
                    <motion.div 
                        className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-2xl border border-slate-200/60 dark:border-slate-700/60 shadow-sm overflow-hidden"
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                    ...
                    >
                        <Tabs value={activeTab} onValueChange={setActiveTab}>
                            <div className="border-b border-slate-200/60 px-4 pt-3 bg-gradient-to-r from-indigo-50/50 to-purple-50/50">
                                <TabsList className="bg-white/80 p-0.5 rounded-xl h-9 grid grid-cols-4 w-full shadow-sm">
                                    <TabsTrigger value="suggestions" className="text-xs rounded-lg data-[state=active]:bg-indigo-600 data-[state=active]:text-white data-[state=active]:shadow-md transition-all">
                                        Suggestions
                                    </TabsTrigger>
                                    <TabsTrigger value="rewrite" className="text-xs rounded-lg data-[state=active]:bg-indigo-600 data-[state=active]:text-white data-[state=active]:shadow-md transition-all">
                                        <Sliders className="w-3 h-3 mr-1" /> Tone
                                    </TabsTrigger>
                                    <TabsTrigger value="analytics" className="text-xs rounded-lg data-[state=active]:bg-indigo-600 data-[state=active]:text-white data-[state=active]:shadow-md transition-all">
                                        <Activity className="w-3 h-3 mr-1" /> Flow
                                    </TabsTrigger>
                                    <TabsTrigger value="stats" className="text-xs rounded-lg data-[state=active]:bg-indigo-600 data-[state=active]:text-white data-[state=active]:shadow-md transition-all">
